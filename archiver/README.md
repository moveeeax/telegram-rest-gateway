...
- `GET /health` — без auth. `200`, пока Kafka-consumer штатно потребляет топик; `503`, если
  ...
  `ARCHIVER_DROP_CIRCUIT` — на этот эндпоинт смотрят и liveness, и readiness пробы k8s, так что
  оба реагируют на нездоровый consumer (рестарт/вывод из ротации), а не только на упавший HTTP.
...
- **Медиа в S3:** если заданы `ARCHIVER_S3_BUCKET` + `ARCHIVER_GATEWAY_TEMPLATE`, при каждом
  ...
  **N подряд drop'ов** (`ARCHIVER_DROP_CIRCUIT`, default 20) — fail-closed: лог с контекстом и
  ...
  разрыва процесса) и возвращается в 200 при успешном автопереподключении. Битый JSON /
