...
> **Видимость пакета.** Пакет `builder` в GHCR должен быть **публичным** — тогда любой PR,
> включая форки, тянет тулчейн без кредов, а токен форка (read-only) не мешает. Секретов в
> образе нет. Если оставить приватным, форк-PR, требующие пересборки builder'а, будут
> скипать C++-джобы (см. «Форки»).
...
4. После пакета сделать пакет `builder` публичным (Packages → builder → Package settings),
   чтобы форк-PR тянули тулчейн без кредов.
